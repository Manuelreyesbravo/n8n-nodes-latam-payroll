...
function chileCalcularFiniquito(sueldoBruto, anosAntiguedad, diasVacacionesPendientes, mesTrabajado) {
    const sueldoDiario = sueldoBruto / 30;
...
function mexicoCalcularVacaciones(sueldoMensual, anosAntiguedad) {
    ...
    if (anosAntiguedad >= 6)
        diasVacaciones = 22;
    if (anosAntiguedad >= 11)
        diasVacaciones = 24;
    if (anosAntiguedad >= 16)
        diasVacaciones = 26;
    if (anosAntiguedad >= 21)
        diasVacaciones = 28;
    if (anosAntiguedad >= 26)
        diasVacaciones = 30;
    if (anosAntiguedad >= 31)
        diasVacaciones = 32;
...
function mexicoCalcularAguinaldo(sueldoMensual, mesesTrabajados = 12) {
    const sueldoDiario = sueldoMensual / 30;
...
function mexicoCalcularFiniquito(sueldoMensual, anosAntiguedad, diasVacacionesPendientes) {
    const sueldoDiario = sueldoMensual / 30;
...
function argentinaCalcularSAC(mejorSueldoSemestre, mesesTrabajados = 6) {
...
function argentinaCalcularIndemnizacion(mejorSueldoUltimoAno, anosAntiguedad, diasVacacionesPendientes) {
...
function colombiaCalcularCesantias(sueldoMensual, diasTrabajados = 360) {
...
function colombiaCalcularLiquidacion(sueldoMensual, anosAntiguedad, diasTrabajadosUltimoAno) {
    const sueldoDiario = sueldoMensual / 30;
...
function peruCalcularCTS(sueldoMensual, mesesTrabajados = 6) {
...
function peruCalcularLiquidacion(sueldoMensual, anosAntiguedad, mesesUltimoAno) {
...
function brasilCalcularDecimoTerceiro(salarioMensual, mesesTrabalhados = 12) {
...
function brasilCalcularRescisao(salarioMensual, mesesTrabalhados, saldoFgts, demissaoSemJustaCausa = true) {
...
function ecuadorCalcularDecimoCuarto(sbu = 460, mesesTrabajados = 12) {
...
function ecuadorCalcularLiquidacion(sueldoMensual, anosAntiguedad, mesesUltimoAno, sbu = 460) {
    const sueldoDiario = sueldoMensual / 30;
...
function espanaCalcularPagasExtras(sueldoMensual, mesesTrabajados = 6) {
...
function espanaCalcularFiniquito(sueldoAnual, anosAntiguedad, diasVacacionesPendientes) {
    const sueldoDiario = sueldoAnual / 365;
...
class LatamPayroll {
    ...
    async execute() {
        ...
        for (let i = 0; i < items.length; i++) {
            const pais = this.getNodeParameter('pais', i);
            ...
            try {
                ...
                if (pais === 'chile') {
                    if (operation === 'sueldo_liquido') {
                        ...
                        const diasMes = this.getNodeParameter('diasMesTrabajado', i, 30);
                ...
                // COLOMBIA
                    ...
                    if (operation === 'sueldo_liquido') {
                        ...
                        const diasTrabajados = this.getNodeParameter('diasTrabajados', i, 360);
                ...
                // BRASIL
                    ...
                    if (operation === 'sueldo_liquido') {
                        ...
                        const saldoFgts = this.getNodeParameter('saldoFgts', i, 0);
                        const semJustaCausa = this.getNodeParameter('semJustaCausa', i, true);
...
exports.LatamPayroll = LatamPayroll;
